...
choose_local(struct tundev *tdev, fd_set *set) {
  ...
  int cnt = 0;
	...
	for (i=0;i<tdev->local_n;i++) {
      ...
      // fprintf(stderr, "[ljj][1] fd:%d is writable\n", tdev->localfd[i]);
			// t += BASE_COUNT + tdev->local_count[i];
      // sum += tdev->local_count[i];
      cnt += 1;
		}
	}
	if (cnt == 0)
		return 0;
	r = random() % cnt;
  // fprintf(stderr, "[ljj] r = %d\n", r);
  cnt = 0;
	t = 0;
	for (i=0;i<tdev->local_n;i++) {
		if (FD_ISSET(tdev->localfd[i], set)) {
      // fprintf(stderr, "[ljj][2] fd:%d is writable\n", tdev->localfd[i]);
      if (cnt++ == r) return i;
      /*
			t += BASE_COUNT + tdev->local_count[i];
			if (r < t) {
...
tun_to_inet(struct tundev *tdev, fd_set *wt) {
  ...
  //fprintf(stderr, "[ljj] choose local: %d\n", localindex);
