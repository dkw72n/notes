...
class JDWPClient:
    ...
    def get_class_by_name(self, name):
        for entry in self.classes:
            #print(entry["signature"].lower(), name.lower(), entry["signature"].lower() == name.lower())
...
def runtime_exec(jdwp, args):
    ...
    print(threads)
    # packagename = getPackageName(jdwp, threads[-1]['threadId'])
    loc = chr( TYPE_CLASS )
    ...
    if args.cmd:
        ...
        print(args.loadlibs)
        for loadlib in args.loadlibs:
          tmpLocation = "/data/local/tmp/" + loadlib
          dstLocation = "/data/data/" + packagename + "/" + loadlib
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loadlib", dest="loadlibs", type=str, metavar="LIBRARYNAME", action='append', 
                        help="Specify library to inject into process load")
    parser.add_argument("--packagename", dest="packagename", type=str, metavar="PACKAGENAME",
